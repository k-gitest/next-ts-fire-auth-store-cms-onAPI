...
async function profileFetcher([url, id]: string[]) {
  return await axios.get(url, { params: { id: id } }).then(res => res.data);
}
...
export const useGetUser = (uid: string | undefined) => {
  ...
  const isError = error;
  const userProfile: User | null = data ? data : null;
...
const Profile = () => {
  ...
  const handleRegister = async () => {
    try {
      ...
      const res = await axios.post(`/api/admin/user`, { user, uid: uid })
      setUser(user)
      router.push('/user')
    }
    catch (err) {
